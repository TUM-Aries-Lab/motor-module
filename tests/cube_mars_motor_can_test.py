...
class TestZeroVelocityIsACommandNotAShutdown:
    """``set_velocity(0)`` must stay inside the control path.

    Routing zero to ``stop()`` made an ordinary command tear MIT mode down
    behind 60 ms of blocking settle time. Any assistive controller crosses
    zero every stride, so that turned normal walking into a repeated
    disable/re-enable cycle that also stalled the caller's loop.
    """

    def test_zero_does_not_tear_down_the_transport(self, motor):
        with (
            patch.object(motor, "stop") as stop,
            patch.object(motor, "_send_velocity_command") as send,
        ):
    ...
    def test_zero_reaches_the_motor_as_a_zero_velocity_mit_command(self, motor):
        with patch.object(motor, "set_mit_mode") as mit:
            motor.set_velocity(velocity_erpm=0)
    ...
    def test_zero_does_not_soft_start(self, motor):
        """A zero command has no direction; pre-spinning it would pick reverse."""
        with (
            patch.object(motor, "_pre_velocity_hook") as hook,
            patch.object(motor, "_send_velocity_command"),
        ):
            motor.set_velocity(velocity_erpm=0)

        hook.assert_not_called()

    def test_nonzero_still_soft_starts_in_the_commanded_direction(self, motor):
        with (
            patch.object(motor, "_pre_velocity_hook") as hook,
            patch.object(motor, "_send_velocity_command"),
        ):
        ...
        assert hook.call_args.args[1] == -1

    def test_teardown_is_still_available_explicitly(self, motor):
        """The behaviour did not disappear -- it moved behind its own name."""
        with (
            patch.object(motor, "_stop_refresh") as stop_refresh,
            patch.object(motor, "_recover_bus_if_needed", return_value=True),
            patch.object(motor, "_send_mit_payload", return_value=True),
            patch.object(motor, "disable_mit_mode") as disable,
            patch("motor_python.cube_mars_motor_can.time.sleep"),
        ):
            motor.stop()

        stop_refresh.assert_called_once()
