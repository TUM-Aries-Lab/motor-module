...
\begin{abstract}
This document describes the CAN Bus communication system developed for the
CubeMars AK60-6 brushless motor used in the lower-limb exosuit project.
The motor is controlled at 1\,Mbit/s over a CAN Bus from an NVIDIA Jetson
Orin AGX computer.  The document explains why CAN was selected over the
initially explored UART interface, covers the hardware and software
configuration required to bring the system to an operational state, and
documents the mechanisms built to achieve reliable and precisely controlled
motor motion.  All technical concepts are introduced from first principles,
making the document accessible to a reader with no prior knowledge of
embedded-systems communication or motor control.
\end{abstract}
...
This document describes how the exosuit communicates with the CubeMars
AK60-6 brushless motor.  The focus is on the communication interface
chosen, the reasoning behind that choice, and the software mechanisms
developed to keep the motor responding reliably and precisely to movement
commands.  These mechanisms address challenges that are not unique to this
motor but are common whenever an embedded computer must send continuous
commands to an actuator over a shared digital network.

All technical terms are introduced and explained when they first appear.
No prior knowledge of embedded-systems communication or motor control is
assumed.

Section~\ref{sec:motivation} compares the two communication interfaces
the motor supports and explains why CAN was chosen.
Section~\ref{sec:hardware} describes the hardware configuration required
on the Jetson Orin.
Section~\ref{sec:protocol} explains how commands are structured and
transmitted.
Section~\ref{sec:feedback} describes the status data the motor
continuously broadcasts in return.
Section~\ref{sec:watchdog} covers the firmware watchdog timer and the
continuous command refresh mechanism built around it.
Sections~\ref{sec:softstart} and~\ref{sec:position} document two
higher-level motion strategies that were designed and evaluated.
Section~\ref{sec:telemetry} presents the real-time telemetry interface
and fault-detection system.
Section~\ref{sec:motion} describes the software PD position controller
used for motion-capture validation, covering direction detection,
CAN noise mitigation, and the sinusoidal sweep protocol.
Section~\ref{sec:service} explains how the CAN interface initialises
automatically at every system boot.
Section~\ref{sec:testing} describes the automated hardware test suite used
to verify the complete communication and control stack against physical hardware.
A full parameter reference closes the document in
...
\subsection{Brake Current Mode}

In addition to the five motion instruction types listed in
Table~\ref{tab:modes}, the motor supports a sixth mode called
\emph{brake current}.  Unlike regular current control, which applies a
torque in a chosen direction so that the motor accelerates or decelerates,
brake current uses regenerative braking: the motor windings are short-circuited
in a controlled manner that converts kinetic energy into heat, forcing the
shaft to slow down rapidly.

The payload encoding is identical to regular current control --- the desired
braking force in Amperes multiplied by 1\,000 and stored as a 32-bit
big-endian integer.  The only difference is in the instruction type byte of
the address, which is set to a distinct value (0x02 instead of 0x01) so
the motor firmware applies the braking action rather than a
proportional torque.  This mode is useful in exosuit applications where
the leg must be actively slowed down under load rather than simply left
to coast.
...
\begin{figure}[H]
  \centering
  \begin{tikzpicture}[
      node distance = 1.2cm and 3.6cm,
      lblock/.style  = {rectangle, rounded corners, draw=black, fill=blue!8,
                        text width=4.8cm, minimum height=1cm,
                        align=center, font=\small},
      rblock/.style  = {rectangle, rounded corners, draw=black, fill=green!10,
                        text width=5.2cm, minimum height=1cm,
                        align=center, font=\small},
      diam/.style    = {diamond, draw=black, fill=orange!15,
                        text width=2.8cm, align=center,
                        font=\small, inner sep=1pt},
      snode/.style   = {rectangle, rounded corners=8pt, draw=black, fill=gray!20,
                        text width=4.8cm, minimum height=1cm,
    ...
    \node[lblock, below=of start]    (store)
      {The instruction and its value are stored internally};
    \node[diam,   below=1.4cm of store] (chk)
      {Background process already running?};
    \node[lblock, below=2.0cm of chk]   (ready)
      {Background process confirmed active};
    \node[lblock, below=of ready]    (main)
      {Main program continues with other tasks};

    %── RIGHT COLUMN: background process ──────────────────────────────────
    \node[rblock, right=of chk]          (launch)
      {Start the background process};
    \node[rblock, right=of ready]         (loop)
      {Every 20\,ms: retransmit the stored instruction over the CAN bus};
    \node[diam,   below=1.4cm of loop]   (stop)
      {Stop command received?};
    \node[snode,  text width=5.2cm, below=1.4cm of stop] (end)
      {Terminate background process,\\send zero-current command};

    %── ARROWS: main program ──────────────────────────────────────────────
    \draw[arr] (start) -- (store);
    \draw[arr] (store) -- (chk);
    \draw[arr] (chk)   -- node[lbl, left]{Yes} (ready);
    \draw[arr] (ready) -- (main);

    %── ARROWS: start background process if not running ───────────────────
    \draw[arr] (chk.east)     -- node[lbl, above]{No} (launch.west);
    \draw[arr] (launch.south) -- (loop.north);  % flows straight down

    %── ARROW: background process signals active to main program ──────────
    \draw[arr] (loop.west) -- (ready.east);     % clean horizontal, no crossings

    %── ARROWS: background loop ───────────────────────────────────────────
    \draw[arr] (loop.south)  -- (stop.north);
    \draw[arr] (stop.east)   -- ++(0.9,0)
                             |- node[lbl, right, pos=0.25]{No\,---\,repeat}
                             (loop.east);
    \draw[arr] (stop.south)  -- node[lbl, right]{Yes} (end.north);

  \end{tikzpicture}
  \caption{The continuous command refresh mechanism.  The left column shows
           the main program: it stores the instruction, checks whether the
           background process is already running, and then continues with
           other tasks.  The right column shows the background process, which
           retransmits the stored instruction every 20\,ms, keeping the
           motor's watchdog satisfied indefinitely.  When a stop command is
           issued, the background process terminates and a zero-current
           command releases the motor.}
  \label{fig:refresh}
\end{figure}
...
\subsection{Note on Firmware Behaviour}

The two-phase strategy described above was the originally designed approach
for position moves.  During subsequent validation testing it was discovered
that, on the specific firmware version installed on the evaluation unit,
velocity mode and position mode commands produce a valid CAN acknowledgement
from the motor but do \emph{not} result in shaft rotation.  The root cause was subsequently characterised precisely.
Velocity-mode frames (arb\_id~$= \texttt{0x0303}$),
position-mode frames (arb\_id~$= \texttt{0x0403}$), and
position-with-velocity frames (arb\_id~$= \texttt{0x0603}$) each use
an arbitration ID that differs from the raw duty-cycle channel
(arb\_id~$= \texttt{MOTOR\_ID} = \texttt{0x03}$). The motor's
watchdog timer monitors \emph{only} the duty-cycle channel; frames
arriving on any other arbitration ID are processed by the relevant
firmware module but do \emph{not} reset the watchdog counter.
As a result, a continuous stream of position or velocity commands
drives the watchdog to expiry after exactly 100\,ms regardless of
firmware version or hardware configuration: the motor silently cuts
drive power and the shaft coasts while the command stream continues
uninterrupted.

As a result, this two-phase strategy could not be deployed in practice.
The working position control approach developed as a replacement is
described in Section~\ref{sec:motion}: a discrete PD controller running
entirely on the Jetson, using duty-cycle commands and the motor's 50\,Hz
position feedback to servo the arm to any target angle in software.
The present section is retained as it documents the originally intended
design and remains applicable if the firmware is reconfigured to reset
the watchdog on all arbitration IDs, or if a production unit with
extended firmware is used.
...
\begin{description}[leftmargin=4.8cm, style=nextline]
  \item[\texttt{get\_position()}]
    Returns the current shaft angle in degrees relative to the position
    recorded at start-up, as a floating-point number.  This is the primary
    feedback quantity used by the PID position controller described in
...
\begin{table}[H]
  ...
  \begin{tabular}{p{1.5cm} p{10cm}}
    ...
    6 & MOSFET over-temperature fault: the motor driver board temperature exceeds the safe limit \\
    7 & Motor lock-up: shaft is mechanically blocked or the rotor has stalled \\
    \bottomrule
  \end{tabular}
\end{table}
...
As characterised in Section~\ref{sec:position}, velocity-mode,
position-mode, and position-with-velocity-mode frames arrive on
arbitration IDs other than \texttt{0x03} and therefore do not reset the
motor's watchdog counter.  Sending any of those command types causes
the watchdog to fire within 100\,ms and cut drive power silently, even
when the motor acknowledges the frames correctly.

The only mode confirmed to produce sustained, controllable rotation is
duty-cycle mode (arb\_id~$= \texttt{MOTOR\_ID} = \texttt{0x03}$),
which resets the watchdog on every frame.  Position control is therefore
implemented entirely in software, running on the Jetson, using the
motor's 50\,Hz position feedback and a discrete PD controller that
outputs a duty-cycle value each tick.

\subsection{The PD Controller}

The controller runs at 50\,Hz (one tick every 20\,milliseconds).  Its
output duty-cycle value $u(t)$ is computed from two terms:

\begin{description}[leftmargin=3.8cm, style=nextline]
  \item[Proportional]
    A correction proportional to the physical angular error between the
    target and the measured position:
    \begin{equation}
      u_P = K_P \cdot e(t), \qquad e(t) = \theta_{\text{target}} - \theta_{\text{actual}}
    \end{equation}
    With $K_P = 0.005\,\text{duty}/\text{deg}$, a 30$^\circ$ error
    produces a 15\,\% duty cycle --- exactly half the maximum output,
    leaving headroom for the derivative braking contribution.

  \item[Derivative (speed-damping term)]
    A term proportional to the signed shaft speed in electrical RPM,
    which damps momentum and resists overshoot:
    \begin{equation}
      u_D = -K_D \cdot \omega_{\text{erpm}}(t) \cdot f_{\text{dir}},
      \qquad K_D = 0.000015,\quad f_{\text{dir}} = +1
    \end{equation}
    Here $\omega_{\text{erpm}}$ is the signed speed reported directly
    by the motor feedback (positive = forward), and $f_{\text{dir}}$
    is the forward-direction flag confirmed by the startup detection
    procedure (Section~\ref{sec:detect_fw_dir}).  At
    3\,000 electrical RPM the derivative contribution is $0.045$ duty
    units, partially countering a 9$^\circ$ proportional error.
\end{description}

The combined output $u = u_P + u_D$ is clamped to
$[-0.30,\,+0.30]$ (30\,\% of supply voltage in either direction).
A dead zone of $\pm 1^\circ$ around the target produces zero output,
preventing continuous small oscillations once the arm is close enough.
A waypoint is considered reached once the position error is below
$5^\circ$, the speed is below 500 electrical RPM, and both conditions
have been maintained for at least 1.5 consecutive seconds
(per-waypoint timeout: 20\,s).

An integral term was deliberately excluded.  Because the cable-driven
arm loads the motor asymmetrically --- gravity acts in only one direction
of swing --- integral accumulation during hold phases built up enough
correction to trigger oscillation when the arm was released on the return
stroke.  The proportional term alone provides sufficient steady-state
tracking for the sinusoidal sweep profile described in
...
\begin{description}[leftmargin=4.2cm, style=nextline]
  \item[Rotation guards]
    Hard limits expressed in physical degrees.  If the arm position
    leaves the window $[-25^\circ,\,+175^\circ]$ the script performs an
    emergency stop immediately: all current is cut, the CAN bus is
    closed, and execution terminates with a log entry.

  \item[Physical soft limits]
    Soft clamp limits of $-20^\circ$ (slightly below the natural hang
    position) and $+190^\circ$ (above the expected operating maximum)
    gate the PD output on every tick: if the arm is at or below
    $-20^\circ$ no downward duty is applied; if at or above
    $+190^\circ$ no upward duty is applied.  These act as a last
    resort before the hard rotation guards fire.
\end{description}

Both guard layers are independent of the PD calculation and apply on
every control tick, including during braking phases.

\subsection{Validation Sequence}
\label{sec:sequence}

The validation run consists of six stages executed exactly once:

\begin{enumerate}
  \item \textbf{find\_home()} --- the motor is set to coast (duty~$= 0$).
    The arm is allowed to hang under gravity while position and speed are
    sampled at 50\,Hz.  Once $|\omega_{\text{erpm}}| < 200$ has been
    maintained for 1.5 consecutive seconds, the resting position is
    recorded as the home reference $\theta_{\text{home}}$.

  \item \textbf{detect\_fw\_dir()} --- a 2\,\% duty-cycle pulse is applied
    for 0.5\,s and the resulting change in physical position is measured.
    If the arm moves in the positive direction, the forward-direction flag
    is set to $f_{\text{dir}} = +1$; if it moves in the negative direction,
    to $-1$.  After the pulse, the arm is re-homed to absorb the small
    displacement.  For the current hardware installation,
    $f_{\text{dir}} = +1$ is confirmed (positive duty raises the arm).
    Full details are given in Section~\ref{sec:detect_fw_dir}.

  \item \textbf{move\_to($120^\circ$)} --- the PD controller drives the arm
    directly from the home position to $120^\circ$ (30° above horizontal)
    without an intermediate stop.  In a representative run this stage
    completed in 5.12\,s with a peak speed of 1\,720 electrical RPM.

  \item \textbf{sine\_sweep()} --- a sinusoidal reference trajectory is
    tracked for 90\,s:
    \begin{equation}
      \theta_{\text{target}}(t)
        = 90 + 30\cos\!\left(\frac{2\pi t}{6}\right)
        \quad [\text{deg}]
    \end{equation}
    This produces a $120^\circ \leftrightarrow 60^\circ$ oscillation
    with a 6\,s period (15 complete cycles in 90\,s). The cosine phase
    places the arm at the $120^\circ$ peak at the start of the sweep and
    swings it down to $60^\circ$ on the first half-cycle.  In a
    representative run the mean tracking error across the sweep was
    $6.6^\circ$ with a peak speed of 5\,280 electrical RPM before the
    speed-brake engaged.

  \item \textbf{Speed-brake block (within sine\_sweep)} --- at any control
    tick where $|\omega_{\text{erpm}}| > 3\,000$ (electrical RPM),
    the duty output is forced to zero and execution pauses until speed
    drops below 1\,000 electrical RPM, after which PD control resumes.
    Ticks during this coast period are logged with
    \texttt{label = "brake"}. Full details are given in
    Section~\ref{sec:speed_brake}.

  \item \textbf{coast\_to\_hang()} --- duty is set to zero and the arm
    coasts under gravity to the natural hang position.  The stage exits
    once $|\omega_{\text{erpm}}| < 300$ has been maintained for
...
file named \texttt{mocap\_YYYY-MM-DD\_HH-MM-SS.csv} in the
\texttt{data/logs/} directory:
...
\begin{center}
  \begin{tabular}{ll}
    ...
    \texttt{commanded\_deg}   & Instantaneous sinusoidal target angle (degrees) \\
    \texttt{actual\_deg}      & Physical arm angle relative to home (degrees) \\
    \texttt{error\_deg}       & Tracking error $e(t) = \theta_{\text{target}} - \theta_{\text{actual}}$ \\
    ...
    \texttt{duty}             & Duty-cycle output from the PD controller \\
    \texttt{label}            & Stage name (e.g.\ \texttt{move\_to\_120},
                                \texttt{sine\_sweep}), or \texttt{"brake"}
                                when the speed-brake coast is active \\
    \bottomrule
  \end{tabular}
\end{center}

After the run completes, a summary table is printed to the console showing
per-stage mean error, maximum error, and peak speed.  A two-panel PNG
figure is saved alongside the CSV: the upper panel shows commanded and
actual angle over time; the lower panel shows shaft speed in electrical
RPM over time, with speed-brake intervals identifiable as the duty
dropping to zero.
\subsection{CAN Bus Noise Drain}
\label{sec:noise_drain}

During position-mode debugging a second device sharing the CAN bus was
found to be broadcasting frames on \texttt{arb\_id = 0x0088} at a very
high rate.  Each frame carries eight zero bytes and has no relation to
the motor.

The original \texttt{read\_feedback()} implementation called
\texttt{bus.recv()} exactly once and returned \texttt{None} if the
arbitration ID did not match the motor's feedback address.  With the
\texttt{0x0088} flood, almost every \texttt{recv()} call consumed a
noise frame, making motor feedback invisible to the control loop (logged
as over 7\,000 consecutive ``no feedback'' ticks while the arm sat
stationary).

The fix is a \emph{deadline-drain} loop: \texttt{recv()} is called
repeatedly within a 15\,ms budget; frames whose arbitration ID does not
match the motor feedback pattern are discarded silently; the loop exits
as soon as a valid motor frame is received or the deadline expires:
...
def read_feedback(bus, deadline_ms=15.0):
    ...
    while True:
        ...
        if msg.arbitration_id not in FEEDBACK_IDS:
            continue        # drain 0x0088 noise
        return parse_feedback(msg)
\end{verbatim}

This pattern is directly analogous to the \texttt{\_refresh\_loop()} in
\texttt{cube\_mars\_motor\_can.py}, which already included a comment
noting the \texttt{0x0088} flood.  The 15\,ms budget is chosen to
consume one full 50\,Hz feedback interval, guaranteeing at least one
valid frame is received if the motor is active.

\subsection{Forward-Direction Detection}
\label{sec:detect_fw_dir}

The cable-driven drivetrain means the relationship between motor shaft
direction and arm movement direction cannot be assumed: it depends on
cable routing, spool geometry, and direction of wind.  For this reason
the script determines the relationship automatically at the start of
every run.

A 2\,\% duty-cycle pulse is applied for 0.5\,s.  This duty level is
below the motor's stall torque but sufficient to overcome bearing
friction, moving the arm a small and measurable distance (typically
$2$--$5^\circ$).  If the physical position increases, the
forward-direction flag is set to $f_{\text{dir}} = +1$ (positive duty
raises the arm); if it decreases, to $-1$.

For the current cable routing on the test rig, $f_{\text{dir}} = +1$
is consistently confirmed: a $+2\,\%$ duty pulse moves the arm
$\approx 3.1^\circ$ in the positive direction.  After the pulse, the
arm is re-homed to absorb this displacement without resetting the
motor's encoder origin.

An earlier version of the test used a 4\,\% duty for 1\,s; this moved
the arm $31^\circ$, making re-homing impractical.  The 2\,\% for 0.5\,s
combination was found to be the minimum reliably distinguishable
displacement.

\subsection{Speed-Brake Safety Net}
\label{sec:speed_brake}

With $K_P = 0.005$ and $\texttt{MAX\_DUTY} = 0.30$, a 30$^\circ$
tracking error produces a 15\,\% duty command.  Over a 90\,s sine sweep,
momentum accumulated across successive cycles until the arm exceeded
5\,280 electrical RPM (at $t \approx 87$\,s), overshot to
$-21^\circ$, and triggered the rotation guard.

Reducing the PD gains to limit maximum speed made the motion visibly
jerky and introduced long delays at the start of each half-cycle;
this was unacceptable for motion-capture recording.

The adopted solution is a \emph{speed-brake block} that does not change
the nominal controller gains:

\begin{itemize}
  \item If $|\omega_{\text{erpm}}| > 3\,000$ during the sine sweep,
    the duty command is set to zero (motor coasts).
  \item Control remains suspended until
    $|\omega_{\text{erpm}}| < 1\,000$.
  \item The PD controller then resumes from the current arm position;
    because there is no integral term, no accumulated correction is
    carried across the coast interval.
\end{itemize}

Speed-brake ticks are logged with \texttt{label = "brake''} for
post-run analysis.  The mechanism fires only at speeds that would lead
to overshoot and does not affect the motion profile during normal
sinusoidal tracking.

\subsection{Comprehensive API Demonstration}

A companion script, \texttt{can\_demo.py}, exercises every public method of
the motor control class in a single sequential run.  The thirteen sections
it covers are: communication check; motor enable; status query; individual
telemetry reads (temperature, current, speed); the combined
\texttt{get\_motor\_data} call; position query; forward and reverse
duty-cycle motion with live ERPM readback; velocity command (acknowledged
but non-functional on this firmware); brake current; tendon actuation;
emergency stop; and the complete error-code reference table.

Running \texttt{python can\_demo.py} after the CAN interface has been
brought up provides a fast end-to-end confirmation that the hardware is
reachable, the CAN filter is working correctly, and all software paths are
exercisable.
% ═══════════════════════════════════════════════════════════════════════════
...
The following mechanisms were developed to make the interface work reliably
...
\begin{description}[leftmargin=4.5cm, style=nextline]
  ...
  \item[Software PD position control]
    Because velocity and position mode commands do not reset the motor
    watchdog (they arrive on different arbitration IDs), a discrete PD
    controller running at 50\,Hz on the Jetson implements position
    control entirely in software using duty-cycle commands and the
    motor's 50\,Hz feedback.  Hard rotation guards and soft physical
    limits clamp motion to a defined angular range.

  \item[CAN noise drain]
    A second device on the bus floods \texttt{arb\_id = 0x0088} at
    high rate.  A deadline-drain loop in \texttt{read\_feedback()}
    discards non-motor frames within a 15\,ms budget, guaranteeing
    motor feedback visibility regardless of bus traffic.

  \item[Forward-direction detection]
    At startup, a 2\,\% duty-cycle pulse of 0.5\,s duration measures
    which shaft direction raises the arm, setting the direction flag
    $f_{\text{dir}}$ automatically.  This makes the script
    cable-routing-agnostic.

  \item[Speed-brake safety net]
    During the 90\,s sinusoidal sweep, if shaft speed exceeds
    3\,000 electrical RPM the duty is forced to zero until speed
    drops below 1\,000 electrical RPM, preventing momentum runaway
    without altering the nominal controller gains.
  ...
  \item[Automated hardware tests]
    A twelve-test pytest suite (\texttt{make test-hardware-can}) verifies
    the complete CAN communication and control stack against physical
    hardware, including bus connectivity, feedback reception, forward and
    reverse spin, telemetry fields, and position commands.  All twelve
    tests pass on the AK60-6~v3 evaluation unit.
...
\begin{table}[H]
  ...
  \begin{tabular}{ll}
    ...
    \midrule
    \multicolumn{2}{l}{\textit{PD position controller (Section~\ref{sec:motion})}} \\
    \midrule
    PD control rate                                & 50\,Hz (every 20\,ms) \\
    Proportional gain $K_P$                        & 0.005\,duty/deg \\
    Derivative gain $K_D$                          & 0.000015\,duty/(electrical RPM) \\
    Forward-direction flag $f_{\text{dir}}$         & $+1$ (confirmed at run start) \\
    Maximum duty cycle                             & $\pm$\,0.30 (30\,\% of supply) \\
    Dead-zone (no output below)                    & $\pm$\,1° \\
    Waypoint settle threshold --- position         & $<5°$ \\
    Waypoint settle threshold --- speed            & $<500$ electrical RPM \\
    Settle hold time before advancing              & 1.5\,s \\
    Per-waypoint timeout                           & 20\,s \\
    Rotation guard --- lower hard limit            & $-25°$ (emergency stop) \\
    Rotation guard --- upper hard limit            & $+175°$ (emergency stop) \\
    Physical soft limit --- lower                  & $-20°$ \\
    Physical soft limit --- upper                  & $+190°$ \\
    \midrule
    \multicolumn{2}{l}{\textit{Sine-sweep validation (Section~\ref{sec:sequence})}} \\
    \midrule
    Sweep centre angle                             & $90°$ \\
    Sweep amplitude                                & $\pm\,30°$ ($120° \leftrightarrow 60°$) \\
    Sweep period                                   & 6\,s \\
    Sweep duration                                 & 90\,s (15 complete cycles) \\
    Speed-brake activation threshold               & 3\,000 electrical RPM \\
    Speed-brake resume threshold                   & 1\,000 electrical RPM \\
    Direction-detect pulse duty                    & 2\,\% for 0.5\,s \\
    CAN noise drain budget                         & 15\,ms deadline per feedback call \\
    Data log rate                                  & 20\,Hz \\
    \bottomrule
  \end{tabular}
\end{table}
