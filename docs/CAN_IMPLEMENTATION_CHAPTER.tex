...
Section~\ref{sec:motion} describes the software PID position controller
used to validate multi-loop arm-sweep trajectories.
...
from the motor but do \emph{not} result in shaft rotation.  The root cause
is not definitively established but is believed to be a firmware
configuration issue specific to the evaluation hardware.
...
described in Section~\ref{sec:motion}: a discrete PID controller running
...
design and remains applicable if the firmware is updated or a production
unit with different firmware is used.
